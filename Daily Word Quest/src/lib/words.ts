// A curated list of common 5-letter words used both as possible answers
// and as the valid-guess dictionary. Kept compact but large enough to
...
export const WORDS: string[] = [
  ...
  "yield","young","youth","zebra","zonal","zones","crane","slate","trace","crate",
  "stare","arose","raise","tales","irate","atone","snare","saint","plate","alone",
];

// Pool used to pick the daily answer. Filter to clean, well-known words.
export const ANSWERS = Array.from(new Set(WORDS.filter((w) => /^[a-z]{5}$/.test(w))));

export function isValidWord(word: string): boolean {
  // Accept any 5-letter alphabetic word. The curated ANSWERS list is small,
  // so restricting guesses to it would reject most real English words.
  return /^[a-z]{5}$/.test(word.toLowerCase());
}

/** Days since 2024-01-01, used as an index into ANSWERS. */
...
export function dailyWord(date = new Date()): string {
  const idx = dayIndex(date) % ANSWERS.length;
  return ANSWERS[(idx + ANSWERS.length) % ANSWERS.length];
}
